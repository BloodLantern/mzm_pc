...
void TextDrawMessageCharacter(u16 charID, u32* dst, u16 indent, u8 color)
{
    ...
    BitFill(3, 0, (void*)gCurrentCharacterGfx, sizeof(gCurrentCharacterGfx), 16);
    width = TextGetCharacterWidth(charID);

    for (pass = 0; pass < 2; pass++)
    {
        if (pass != 0)
            dstGfx = ((u32*)gCurrentCharacterGfx) + 16;
        else
            dstGfx = ((u32*)gCurrentCharacterGfx);
 ...
 *
 * @param locationTextLocation text
 * @param gfxSlotGraphics slot
 */
...
u8 TextProcessFileScreenPopUp(void)
{
    ...
    const u16* const * const *fileScreenTextPointers;
    s32 tmp;
